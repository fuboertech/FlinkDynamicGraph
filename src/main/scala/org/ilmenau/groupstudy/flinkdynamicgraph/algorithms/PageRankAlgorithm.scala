...
object PageRankAlgorithm {
  ...
  def runClassic(graph: Graph[Integer, Airport, Integer], border: org.apache.flink.api.java.DataSet[scala.Tuple2[Integer, DoubleValue]] = null): Seq[(Integer, DoubleValue)] = {
    val result = graph.run(new org.ilmenau.groupstudy.flinkdynamicgraph.algorithms.PageRank[Integer, Airport, Integer](0.85, 10000, 0.001, border)).collect().asScala.toSeq
    result.map(f => Tuple2[Integer, DoubleValue](f.getVertexId0, f.getPageRankScore))
  }

  // TODO: Not working properly 
  def runDynamic(graph: Graph[Integer, Airport, Integer], addedEdges: Seq[Edge[Integer, Integer]], firstPageRank: Seq[(Integer, DoubleValue)], env: ExecutionEnvironment): Seq[(Integer, DoubleValue)] = {
