...
class PageRankAlgorithm {
  ...
  def runDynamic(graph: Graph[Integer, Integer, Integer], addedEdges: Seq[Edge[Integer, Integer]], env: ExecutionEnvironment): Seq[(Integer, DoubleValue)] = {
    ...
    while (vc.nonEmpty) {
      ...
      //vc.foreach(println)
      vq = vq.union(vc)
      var vcc = graph.getEdgeIds()
        .filter(e => vc.contains(e._1)).filter(e => vu.contains(e._2)).distinct()
      //println("vcc size:" + vc.size)
      //vcc.collect().foreach(x => println("vcc: "+ x))
      vc = vcc
        .map(e => e._2).collect()
    ...
    val childrens = graph.getEdgeIds()
      .filter(e => vq.contains(e._2)).filter(e => vu.contains(e._1))
      .map(e => e._1).distinct().collect()
    vu = vu.diff(childrens)
    vb = vb.union(childrens)
//    output.append("\nvu: " + vu.sortBy(f => f.intValue()))
...
//    })
    ...
    println("--- Size of subgraph: " + q.size + " and full graph: " + _FullPageRank.size + "subgraph e:" + subgraph.getEdges.count())
...
//        p._2.setValue(p._2.getValue * vq.size /graph.numberOfVertices())
//    })
    val subgraphPRVertices = subgraphPageRank.map(p => p._1)
    val fullPageRank = _FullPageRank.filterNot(t => subgraphPRVertices.contains(t._1)).union(subgraphPageRank)
