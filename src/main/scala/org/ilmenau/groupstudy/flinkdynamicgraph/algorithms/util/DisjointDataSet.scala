...
class DisjointDataSet[K >: Null : ClassTag](val matches: mutable.HashMap[K, K] = mutable.HashMap.empty) {
  ...
  def union(e1: K, e2: K):Unit ={
    ...
    if (dist1 > dist2) {
      ...
      matches.foreach(k => if (k._2.equals(root1)) matches.put(k._1, root2))
    } else {
      matches.put(root2, root1)
      ranks.put(root1, dist1 + 1)
      matches.foreach(k => if (k._2.equals(root2)) matches.put(k._1, root1))
    }
  }
  ...
  /**
    ...
    * @param other
    */
  def merge(other: DisjointDataSet[K]) {
